    ...
    <div class="py-8">
        <div class="max-w-xl mx-auto px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6 text-gray-900 dark:text-gray-100">
                ...
                <form method="POST" action="{{ route('entrate.web.update', $entrata->id) }}" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <input type="date" name="date" id="date" required
                            value="{{ old('date', $entrata->date->format('Y-m-d')) }}"
                    ...
                    <div>
                        ...
                        <input type="number" name="amount" id="amount" required step="0.01" min="0.01"
                            value="{{ old('amount', $entrata->amount) }}"
                            class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100 focus:ring focus:ring-indigo-400" />
                        @error('amount')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="category_id" id="category_id" required
                            ...
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}" @selected(old('category_id', $entrata->category_id) == $category->id)>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="description" id="description" required
                            value="{{ old('description', $entrata->description) }}"
                            class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100 focus:ring focus:ring-indigo-400" />
                        @error('description')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <textarea name="notes" id="notes" rows="3"
                            class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100 focus:ring focus:ring-indigo-400">{{ old('notes', $entrata->notes) }}</textarea>
                        @error('notes')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
