        ...
        <div class="max-w-7xl mx-auto px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6 text-gray-900 dark:text-gray-100">
                ...
                <div class="flex justify-center mb-6">
                    <a href="{{ route('spese.web.create') }}"
                        class="px-5 py-2 text-lg font-bold text-white bg-red-600 rounded hover:bg-red-700 transition">
                        ➕ Aggiungi Nuova Spesa
                    </a>
                </div>

                {{-- 🔍 Filtro --}}
                <form method="GET" action="{{ route('spese.web.index') }}"
                    ...
                    <table class="min-w-full text-sm divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tr>
                                @foreach(['date' => 'Data', 'description' => 'Descrizione', 'amount' => 'Importo'] as $key => $label)
                                    <th class="px-4 py-2 bg-gray-50 dark:bg-gray-700 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                                        <a href="{{ route('spese.web.index', array_merge(request()->query(), [
                                            'sort_by' => $key,
                        ...
                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            @forelse($spese as $spesa)
                                <tr>
                                    <td class="px-4 py-2">{{ $spesa->date?->format('d/m/Y') }}</td>
                                    <td class="px-4 py-2">{{ $spesa->description }}</td>
                                    <td class="px-4 py-2">{{ number_format($spesa->amount, 2, ',', '.') }} €</td>
                                    <td class="px-4 py-2">{{ $spesa->category->name ?? '-' }}</td>
                                    <td class="px-4 py-2 max-w-xs truncate">{{ $spesa->notes ?? '-' }}</td>
                                    <td class="px-4 py-2 space-x-2 whitespace-nowrap">
                                        <a href="{{ route('spese.web.show', $spesa->id) }}"
                                        ...
                                        <form action="{{ route('spese.web.destroy', $spesa->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
